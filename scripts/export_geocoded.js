...
// Load JSON datasets to get original station_id, station_name, and route_ids
...
// Build maps: prefixed_station_id → { original_station_id, station_name, route_ids }
  ...
  for (const { prefix, data } of datasets) {
    for (const route of data.routes) {
      ...
      for (const s of route.stations) {
        ...
        if (!map[sid]) {
          map[sid] = {
            original_station_id: s.station_id,
            station_name: s.station_name,
            route_ids: [],
          };
        }
        if (!map[sid].route_ids.includes(routeId)) {
          map[sid].route_ids.push(routeId);
        }
      }
    }
  }
...
(async () => {
  ...
  const dest = path.resolve(__dirname, '..', 'src', 'data', 'geocoded_stations.json');
  fs.writeFileSync(dest, JSON.stringify(out, null, 2), 'utf-8');
  console.log(`${rows.length} stations géocodées écrites dans ${dest}`);
  await pool.end();
})();
